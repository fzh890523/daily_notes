...
# lua 网上字符串库收藏
> ref: [lua实现的常用string库](http://www.oschina.net/code/snippet_84176_683)
...
# 启动hbase thrift Server/res/t Server
fg启动：
* thrift `hbase thrift start -p <port> --infoport <infoport>`
* rest `hbase rest start -p <port> --infoport <infoport>`

daemon启动：
* thrift `bin/hbase-daemon.sh start thrift`
`/usr/hdp/current/hbase-master/bin/hbase-daemon.sh start thrift -p <port> --infoport <infoport>`
* rest `/usr/hdp/current/hbase-master/bin/hbase-daemon.sh start rest -p <port> --infoport <infoport> `
