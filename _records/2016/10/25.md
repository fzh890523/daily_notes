...
# 摘录： 《Redis、Memcached、Guava、Ehcache中的算法》 主要是LRU
> ref: [Redis、Memcached、Guava、Ehcache中的算法](http://calvin1978.blogcn.com/articles/lru.html)
简单的过了一遍常用cache（external、in-memory）的LRU实现。

# 摘录 《缓存、缓存算法和缓存框架简介》

因为想搞个java的IN-MEMORY-CACHE，所以了解了一下。
> ref: [缓存、缓存算法和缓存框架简介-伯乐在线](http://blog.jobbole.com/30940/)
> ref: [缓存算法](http://www.leexiang.com/cache-algorithm)

```
原文出处： jtraining   译文出处：Lixiang   
引言
...
# spring cache 初步了解
> ref: [注释驱动的 Spring cache 缓存介绍](https://www.ibm.com/developerworks/cn/opensource/os-cn-spring-cache/)
> ref: [Can I set a TTL for @Cacheable](http://stackoverflow.com/questions/8181768/can-i-set-a-ttl-for-cacheable)
> ref: [settings spring @cacheable to cache for 10 seconds](http://stackoverflow.com/questions/19768481/settings-spring-cacheable-to-cache-for-10-seconds)
...
不足：
* 不支持expire，不过可以使用第三方adapter


# log4j报错

```
:12,966 ERROR Loggers cannot be configured without a name
2016-10-25 20:27:12,967 ERROR Logger contains an invalid element or attribute "title"
2016-10-25 20:27:12,967 ERROR Loggers cannot be configured without a name
2016-10-25 20:27:12,970 ERROR Null object returned for Logger in Loggers.
2016-10-25 20:27:12,971 ERROR Null object returned for Logger in Loggers.
2016-10-25 20:27:12,981 ERROR RollingFile contains an invalid element or attribute "title"
2016-10-25 20:27:12,981 ERROR No name provided for FileAppender
2016-10-25 20:27:12,983 ERROR RollingFile contains an invalid element or attribute "title"
2016-10-25 20:27:12,983 ERROR No name provided for FileAppender
2016-10-25 20:27:12,984 ERROR Null object returned for RollingFile in Appenders.
2016-10-25 20:27:12,984 ERROR Null object returned for RollingFile in Appenders.
2016-10-25 20:27:12,984 ERROR Unable to locate appender ROOTFILE for logger 
log4j:WARN No appenders could be found for logger (org.springframework.web.context.support.StandardServletEnvironment).
log4j:WARN Please initialize the log4j system properly.
log4j:WARN See http://logging.apache.org/log4j/1.2/faq.html#noconfig for more info.
2016-10-25 20:27:13,754 ERROR property contains an invalid element or attribute "title"
2016-10-25 20:27:13,755 ERROR Property name cannot be null
2016-10-25 20:27:13,757 ERROR Logger contains an invalid element or attribute "title"
2016-10-25 20:27:13,757 ERROR Loggers cannot be configured without a name
2016-10-25 20:27:13,759 ERROR Logger contains an invalid element or attribute "title"
2016-10-25 20:27:13,759 ERROR Loggers cannot be configured without a name
2016-10-25 20:27:13,772 ERROR Null object returned for Logger in Loggers.
2016-10-25 20:27:13,772 ERROR Null object returned for Logger in Loggers.
2016-10-25 20:27:13,774 ERROR RollingFile contains an invalid element or attribute "title"
2016-10-25 20:27:13,774 ERROR No name provided for FileAppender
2016-10-25 20:27:13,776 ERROR RollingFile contains an invalid element or attribute "title"
2016-10-25 20:27:13,776 ERROR No name provided for FileAppender
2016-10-25 20:27:13,777 ERROR Null object returned for RollingFile in Appenders.
2016-10-25 20:27:13,777 ERROR Null object returned for RollingFile in Appenders.
2016-10-25 20:27:13,777 ERROR Unable to locate appender ROOTFILE for logger 
```


噗，因为 `title` 需要换成 `name`。
...
> 前面的几行报错要注意： 应该是这个导致spring的报错没有打出来
...
于是，打了端点跟到异常处，然后 **evaluate执行** `ex.printStackTrace()` 打出异常堆栈，如下：
```
org.springframework.beans.factory.parsing.BeanDefinitionParsingException: Configuration problem: 'bean', 'local' or 'parent' is required for <ref> element
Offending resource: class path resource [applicationContext.xml]
    ...
    at org.springframework.beans.factory.xml.BeanDefinitionParserDelegate.parsePropertySubElement(BeanDefinitionParserDelegate.java:1039)
    at org.springframework.beans.factory.xml.BeanDefinitionParserDelegate.parseCollectionElements(BeanDefinitionParserDelegate.java:1202)
    at org.springframework.beans.factory.xml.BeanDefinitionParserDelegate.parseListElement(BeanDefinitionParserDelegate.java:1178)
...
```
这里可以看到是 `ListFactoryBean` ref了一个null的bean。
这样对照配置文件看一下就大约知道是哪些bean没有加载成功了。
发现是：
```xml
    <util:list id="ipCounterProcessorList">
        <ref bean=""/>
    </util:list>
```
这个忘了写。
...
# 踩坑： idea run/debug configuration里的active profiles vs maven窗口的profiles勾选项

在run/debug configuration里指定在launch之前run maven goal `validate` 然后在指定profile的validate goal里做了copy resource。
发现：
* 使用`maven窗口的profiles勾选项`选中该profile，可以触发执行
* 使用active profiles填入该profile，不能触发执行
